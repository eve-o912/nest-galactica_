...
// Pure WDK implementation without Privy dependencies
export class PureWDKWallet {
  private account: any;
  private chain: Chain;
...
export class PureWDKManager {
  private static instances: Map<string, PureWDKWallet> = new Map();
  ...
  static getWallet(config: {
    ...
    const wallet = new PureWDKWallet({
  ...
  static createWallet(config: {
    ...
    return {
      wallet,
      mnemonic,
      privateKey: (wallet as any).account.privateKey,
    };
  }
  ...
  private static getDefaultRpcUrl(chain: string): string {
    const urls = {
  ...
  static getAllWallets(): Map<string, PureWDKWallet> {
